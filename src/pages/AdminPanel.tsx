  ...
  return (
    <div className="min-h-screen bg-gray-50 text-gray-900">
      ...
      <main className="p-4 max-w-6xl mx-auto">
        <Tabs value={activeTab} onValueChange={setActiveTab}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              {languages.map(l => (
                <Card key={l.id} className="bg-white border-gray-200 shadow-sm"><CardContent className="p-4 flex justify-between items-center">
                  <div className="flex items-center gap-2">{l.avatar && <img src={l.avatar} className="w-8 h-8 rounded-full border border-gray-200" />}<span className="font-semibold text-gray-800">{l.name}</span></div>
              ...
              {groups.map(g => (
                <Card key={g.id} className="bg-white border-gray-200"><CardContent className="p-4">
                  <div className="flex justify-between border-b pb-4 mb-4">
                    <div className="flex items-center gap-2">
                      <Users className="w-5 h-5 text-[#1a3673]"/>
                  ...
                  <div className="border-t pt-4 flex justify-between items-center">
                    ...
                    <Button size="sm" variant="outline" className="text-[#1a3673] border-[#1a3673]" onClick={() => { setEditingGroup(g); setShowAddStudentDialog(true); }}><Plus className="w-3 h-3 mr-1" />Add</Button>
                  </div>
            ...
            <div className="grid grid-cols-1 gap-2">
              {books.map(b => (
                <Card key={b.id} className="bg-white"><CardContent className="p-3 flex justify-between items-center text-gray-800 font-medium">
                  <div className="flex items-center gap-3">{b.avatar && <img src={b.avatar} className="w-8 h-10 object-cover rounded border"/>}<div className="flex flex-col"><CardTitle className="text-sm">{b.name}</CardTitle><span className="text-xs text-gray-400">{languages.find(l => l.id === b.languageId)?.name}</span></div></div>
                  <div className="flex gap-2">
          ...
          <TabsContent value="students">
            {students.map(s => <Card key={s.id} className="bg-white mb-2 shadow-sm"><CardContent className="p-3 flex justify-between items-center text-gray-800">
      ...
      <Dialog open={showLanguageDialog} onOpenChange={setShowLanguageDialog}><DialogContent className="bg-white text-gray-900">
        ...
        <DialogFooter><Button onClick={handleSaveLanguage} className="bg-[#1a3673] text-white w-full">Save</Button></DialogFooter></DialogContent>
      </Dialog>
      ...
      <Dialog open={showBookDialog} onOpenChange={setShowBookDialog}><DialogContent className="bg-white text-gray-900">
        ...
        <div className="space-y-4">
          ...
          <div><Label className="text-gray-700">Assign Language</Label><Select value={bookLanguageId} onValueChange={setBookLanguageId}><SelectTrigger className="text-gray-900 border-gray-300 bg-white"><SelectValue placeholder="Select Language" /></SelectTrigger><SelectContent className="bg-white border-gray-200">{languages.map(l => <SelectItem key={l.id} value={l.id}>{l.name}</SelectItem>)}</SelectContent></Select></div>
        </div>
        <DialogFooter><Button onClick={handleSaveBook} className="bg-[#1a3673] text-white w-full">Save</Button></DialogFooter></DialogContent>
      </Dialog>
      ...
      <Dialog open={showSessionDialog} onOpenChange={setShowSessionDialog}>
        <DialogContent className="max-w-2xl bg-white text-gray-900">
          <DialogHeader><DialogTitle className="text-[#1a3673] flex justify-between items-center">Unit Content <Button size="sm" className="bg-[#c5a059] text-white" onClick={() => { if(selectedUnitForSession) Database.addSessionToUnit(selectedUnitForSession.id, { name: 'New Session', emoji: '🎮', order: selectedUnitForSession.sessions.length + 1 }).then(loadData); }}>+ Add Session</Button></DialogTitle></DialogHeader>
          <ScrollArea className="h-80 mt-4 pr-4">
            {selectedUnitForSession?.sessions.sort((a,b)=>a.order-b.order).map(s => (
              <div key={s.id} className="p-3 border-b border-gray-100 flex justify-between items-center">
                <span className="font-semibold text-gray-900">{s.emoji} {s.name}</span>
                <div className="flex gap-2">
      ...
      <Dialog open={!!editingSession} onOpenChange={() => setEditingSession(null)}>
        <DialogContent className="max-w-5xl bg-white text-gray-900 max-h-[90vh] overflow-y-auto">
          ...
          <DialogFooter className="mt-6"><Button onClick={handleSaveSession} className="bg-[#1a3673] text-white w-full h-12">Save Changes</Button></DialogFooter>
        </DialogContent>
      </Dialog>
