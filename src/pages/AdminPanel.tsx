      ...
      <main className="p-4 max-w-6xl mx-auto">
        <Tabs value={activeTab} onValueChange={setActiveTab}>
          ...
          <TabsContent value="units">
            ...
            {languages.map(lang => (
              <div key={lang.id} className="mb-6">
                <h2 className="text-lg font-bold text-[#1a3673] border-b pb-2 mb-4">{lang.name}</h2>
                {books.filter(b => b.languageId === lang.id).map(book => (
                  <div key={book.id} className="ml-4 mb-4">
                    <h3 className="font-semibold text-gray-600 mb-2 flex items-center gap-2"><BookOpen className="w-4 h-4"/>{book.name}</h3>
                    <div className="grid gap-2">
                      {units.filter(u => u.bookId === book.id).map(u => (
                        <div key={u.id} className="p-3 bg-white rounded border flex justify-between items-center text-gray-800">
                          <span>{u.name}</span>
