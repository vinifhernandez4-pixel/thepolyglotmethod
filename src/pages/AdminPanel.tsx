...
export default function AdminPanel({ onLogout }: AdminPanelProps) {
  ...
  const handleBulkUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    for (const file of fileArray) {
      if (file.name.endsWith('.html') && !file.name.includes('_anki')) {
        ...
        const emoji = EMOJI_OPTIONS.find(e => baseName.toLowerCase().includes('preview') ? e === '👀' : baseName.toLowerCase().includes('review') ? e === '📝' : e === '🎮') || '🎮';
        await Database.addSessionToUnit(selectedUnitForSession.id, { name: baseName.charAt(0).toUpperCase() + baseName.slice(1), emoji, "htmlContent": content, "ankiCards": [], order: selectedUnitForSession.sessions.length + 1 });
      }
    }
    await loadData(); setShowBulkUploadDialog(false);
  };
  ...
  const handleDelete = async () => {
    ...
    if (itemToDelete.type === 'language') await Database.deleteLanguage(itemToDelete.id);
    else if (itemToDelete.type === 'book') await Database.deleteBook(itemToDelete.id);
    else if (itemToDelete.type === 'unit') await Database.deleteUnit(itemToDelete.id);
    else if (itemToDelete.type === 'group') await Database.deleteGroup(itemToDelete.id);
    else if (itemToDelete.type === 'user') await Database.deleteUser(itemToDelete.id);
    else if (itemToDelete.type === 'session') await Database.deleteSession(itemToDelete.id);
    ...
    <div className="min-h-screen bg-gray-50 p-4">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
              ...
              return (
                <Card key={g.id}><CardContent className="p-4">
                  ...
                  <div className="border-t pt-4 mt-4 flex justify-between items-center">
                    ...
                    <Button size="sm" onClick={() => { setEditingGroup(g); setShowAddStudentDialog(true); }}><Plus className="w-3 h-3 mr-1" />Add</Button>
                  </div>
        ...
        <TabsContent value="units">
          <Button onClick={() => setShowUnitDialog(true)} className="mb-4">Add Unit</Button>
          {units.map(u => <div key={u.id} className="p-2 border-b flex justify-between"><span>{u.name}</span><div className="flex gap-2"><Eye className="w-4 h-4 cursor-pointer" onClick={() => { setSelectedUnitForSession(u); setShowSessionDialog(true); }} /><Trash2 className="w-4 h-4 cursor-pointer" onClick={() => { setItemToDelete({type: 'unit', id: u.id}); setShowDeleteConfirm(true); }} /></div></div>)}
        </TabsContent>
        <TabsContent value="students">
          <div className="grid gap-2">{students.map(s => <div key={s.id} className="p-2 border bg-white rounded flex justify-between"><span>{s.name} ({s.email})</span><Trash2 className="w-4 h-4 cursor-pointer text-red-500" onClick={() => {setItemToDelete({type: 'user', id: s.id}); setShowDeleteConfirm(true);}} /></div>)}</div>
        </TabsContent>
      </Tabs>
