...
export default function AdminPanel({ onLogout }: AdminPanelProps) {
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-4">
      ...
      }}>+ Add</Button></div><ScrollArea className="h-80">{selectedUnitForSession?.sessions.map(s => <div key={s.id} className="p-3 border-b flex justify-between items-center"><span>{s.name}</span><div className="flex gap-2"><Button size="sm" variant="outline" onClick={() => { setEditingSession(s); setSessionName(s.name); setSessionEmoji(s.emoji); setSessionHtml(s.htmlContent); setSessionAnki(JSON.stringify(s.ankiCards, null, 2)); setShowSessionDialog(false); }}><Edit2 className="w-3 h-3"/></Button><Button size="sm" variant="outline" onClick={() => {setItemToDelete({type:'session', id:s.id}); setShowDeleteConfirm(true);}}><Trash2 className="w-3 h-3"/></Button></div></div>)}</ScrollArea></DialogContent></Dialog>
