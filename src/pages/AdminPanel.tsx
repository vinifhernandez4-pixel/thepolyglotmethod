                  ...
                  <div className="mb-4">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {units.filter(u => u.bookId === g.bookId).map(u => (
                        <Button key={u.id} size="sm" variant={g.unlockedUnitIds.includes(u.id) ? "default" : "outline"} className={g.unlockedUnitIds.includes(u.id) ? "bg-emerald-600 text-white" : "text-gray-500 border-gray-200"} onClick={() => g.unlockedUnitIds.includes(u.id) ? Database.lockUnitForGroup(g.id, u.id).then(loadData) : Database.unlockUnitForGroup(g.id, u.id).then(loadData)}>
